...
<script>
   document.getElementById('program').addEventListener('change', function() {
    fetchSchedules(); // Trigger schedule fetching on program change
});

document.getElementById('year-level').addEventListener('change', function() {
    fetchSchedules(); // Also fetch when year level changes
});
...
document.getElementById('preEnrollmentForm').addEventListener('submit', function(event) {
    ...
    .then(data => {
        if (data.success) {
            ...
            window.location.href = '/newsfeed'; // Redirect or do something on success
        } else {
            alert(data.error || 'Failed to submit pre-enrollment.');
        }
    })
    .catch(error => {
