...
paginatedSubjects.forEach(subject => {
    const departmentColor = departmentColors[subject.schedule.program.code] || '#607D8B'; // Default color

    // Check if there are no students enrolled
    const isEnrolled = subject.subjectEnrolledId !== null;
    const viewGradesButton = isEnrolled 
        ? `<a href="/teacher/subject/grades/${subject.subjectEnrolledId}" class="btn btn-primary btn-view-grades" style="background-color: ${departmentColor}; border: none;">
                View Grades
           </a>`
        : `<button class="btn btn-secondary" disabled style="cursor: not-allowed;">No Enrolled Students</button>`;

    subjectsContainer.innerHTML += `
        <div class="col-md-4">
            <div class="card mb-3 shadow-sm subject-card" style="border-left: 5px solid ${departmentColor}; background-color: #f9f9f9; border-radius: 5px;">
                <div class="card-header text-white" style="background-color: ${departmentColor}; border-radius: 8px 8px 0 0; padding: 10px;">
                    <h5 class="card-title mb-0" style="font-size: 1.2rem; font-weight: bold;">
                        ${subject.schedule.program.code} - ${subject.schedule.subject.subject_code} - ${subject.schedule.section.name}
                    </h5>
                </div>
